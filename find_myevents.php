<?php
    ...
    function find_myevents(mysqli $conn, $mode) {
      ...
      $sql = "SELECT Activity_ID FROM applies
              WHERE Volunteer_Username = '$username' AND Activity_ID NOT IN
              (SELECT Activity_ID FROM hires WHERE Volunteer_Username = '$username')";
      ...
      if ($result->num_rows > 0){
        ...
        while($row = $result->fetch_assoc()) {
            ...
            if($record[2] >= date('Y-m-d')) {
                ...
                $RecPos = $RecPos +113;
                $ActNamePos = $ActNamePos + 113;
                $DurationPos= $DurationPos + 113;
                $LocPos = $LocPos + 113;
